...
View SpireOfBones::lobby() {
  ...
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::hallway_to_spiral)) {
    ...
    if (!this->item_rope) {
      ...
      ADD_OPT(v, "Exit dungeon", this->exit_dungeon());
    }
  } else {
...
View SpireOfBones::hallway_to_spiral() {
  ...
  ADD_OPT(v, "Try the door",
          this->set_room(SAME_ROOM(SpireOfBones::hallway_to_spiral_try_door)));
...
View SpireOfBones::hallway_to_spiral_try_door() {
  ...
  if (this->held_item == ITEM_BOSS_KEY) {
    v.desc = "You unlock the door using your key. It creaks open and you enter "
             "inside.";
    ADD_OPT(v, "Continue", this->set_room(AS_ROOM(SpireOfBones::boss_room)));
...
View SpireOfBones::spiral_staircase() {
  ...
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::hallway_to_spiral)) {
    ...
    ADD_OPT(v, "Go back out",
            this->set_room(AS_ROOM(SpireOfBones::hallway_to_spiral)));
    ADD_OPT(v, "Ascend", this->set_room(AS_ROOM(SpireOfBones::top_of_spiral)));
    ADD_OPT(v, "Descend", this->set_room(AS_ROOM(SpireOfBones::bot_of_spiral)));
  }
...
View SpireOfBones::top_of_spiral() {
  ...
  ADD_OPT(v, "Pull the lever", [this]() {
    ...
    std::function<void()> func = this->set_room(
        SAME_ROOM(SpireOfBones::top_of_spiral_just_pulled_lever));
    func();
  });
  ...
  ADD_OPT(v, "Examine the far end of the hallway",
          this->set_room(AS_ROOM(SpireOfBones::top_of_shaft)));
  return v;
}
...
View SpireOfBones::skeleton_arena_1() {
  ...
  if (this->arrows > 0) {
    ...
    ADD_OPT(v, "Strike back with your fist",
            this->set_room(SAME_ROOM(SpireOfBones::skeleton_arena_1_strike)));
    ADD_OPT(v, "Dodge out of the way",
            this->set_room(SAME_ROOM(SpireOfBones::skeleton_arena_1_dodge)));
    if (this->held_item == ITEM_BOW) {
      ADD_OPT(v, "Swing with your bow",
              this->set_room(SAME_ROOM(SpireOfBones::skeleton_arena_1_swing)));
    }
  }
  return v;
}
  ...
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::top_of_spiral)) {
    ...
    if (this->held_item == ITEM_LODESTONE) {
      v.desc += " Your lodestone illuminates the hallway, allowing you to "
                "check your footing.";
      ADD_OPT(v, "Push forward",
              this->set_room(AS_ROOM(SpireOfBones::axe_room)));
      ADD_OPT(v, "Go back",
              this->set_room(AS_ROOM(SpireOfBones::top_of_spiral)));
    } else {
      ...
      ADD_OPT(v, "Push forward",
              this->set_room(AS_ROOM(SpireOfBones::fell_through_the_trap)));
      ADD_OPT(v, "Go back",
              this->set_room(AS_ROOM(SpireOfBones::top_of_spiral)));
    }
  }
...
View SpireOfBones::top_of_shaft() {
  ...
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::down_in_shaft)) {
    v.desc = "You climb back up the shaft and put the rope back in your bag.";
  }
  if (this->held_item == ITEM_ROPE) {
    ADD_OPT(v, "Climb down the shaft",
            this->set_room(AS_ROOM(SpireOfBones::down_in_shaft)));
  }
  ADD_OPT(v, "Throw a pebble down the shaft", [this]() {
    this->thrown_pebbles =
        (this->thrown_pebbles == 3) ? 3 : this->thrown_pebbles + 1;
    std::function<void()> func =
        this->set_room(SAME_ROOM(SpireOfBones::top_of_shaft_throw_pebble));
    func();
  });
...
View SpireOfBones::top_of_shaft_throw_pebble() {
  ...
  ADD_OPT(v, "Okay", this->set_room(SAME_ROOM(SpireOfBones::top_of_shaft)));
  return v;
}
...
View SpireOfBones::bot_of_spiral() {
  ...
  if (!this->lever_pulled_1 && !this->lever_pulled_2) {
    ...
    if (this->held_item == ITEM_BOW && this->arrows > 0) {
      ADD_OPT(v, "Fire an arrow",
              this->set_room(SAME_ROOM(SpireOfBones::kill_croc)));
    }
  }
  return v;
}
  ...
  if (this->raven_trade) {
    ...
    if (this->thrown_pebbles > 0) {
      ...
      ADD_OPT(v, "Try the pebbles",
              this->set_room(SAME_ROOM(SpireOfBones::stone_room_trade)));
    }
  }
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::croc_pit)) {
...
View SpireOfBones::skeleton_arena_2() {
  ...
  if (this->prev_room.name == ROOM_NAME(SpireOfBones::goblin_lair)) {
    ...
    ADD_OPT(v, "Continue", this->set_room(AS_ROOM(SpireOfBones::stone_room)));
